...
def tag_answers_img(img, form_path: str, answers: list, form_type: int, out_dir: str) -> None:
	def draw_line(img, x, y, w, h):
		cv2.line(img, (x, y), (x+w, y), (0, 0, 0), h)
	q_w = 850
	q_h = 200
	# Inicio de cada questão
	q_i = [670 ,840 ,1040 ,1200 ,1400 ,1580]
	if form_type == 0:
		for i, l in enumerate(q_i):
			draw_line(img, 950 + 450*answers[i], l+175, 80, 15)
			
		# Inicio das questões de sim ou não
		q7_i = 1770
		draw_line(img, 930 + 325*answers[6], q7_i+160, 80, 15)
		q8_9_i = 2100
		draw_line(img, 130 + 325*answers[7], q8_9_i+160, 80, 15)
		draw_line(img, 1430 + 325*answers[8], q8_9_i+160, 80, 15)
		# Inicio da questão de nota
		rating_i = 2270
		...
		for i, l in enumerate(q_i):
			draw_line(img, 900 + increment_table[answers[i]], l+175, 80, 15)
		# Inicio das questões de sim ou não
		q7_i = 1770
		draw_line(img, 910 + 350*answers[6], q7_i+140, 80, 15)
		q8_9_i = 2100
		draw_line(img, 115 + 350*answers[7], q8_9_i+120, 80, 15)
...
def catch_forms_data(forms: list, tag_out_dir: str) -> list:
	...
	for form in forms:
		...
		if tag_out_dir != "":
			tag_answers_img(img, form, r, form_type,tag_out_dir)
